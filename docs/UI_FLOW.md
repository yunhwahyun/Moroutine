...
- 학습영역: 오늘 복습할 단어 목록(`useTodayStudyWords()`) 중 **첫 번째 단어 카드만** 표시하고
  나머지는 "+N" 카운트로만 보여준다(2026-09-16 — 예전엔 목록 전체를 가로 스와이프
  슬라이드로 렌더링했는데, 복습할 단어가 많으면(100개 이상) 카드가 전부 DOM에 올라가 스크롤이
  버벅이고 멈추는 문제가 있었다, 실사용자 리포트). **학습하기/Quiz 시작/자동재생 시작은 이
  미리보기와 무관하게 항상 목록 전체를 대상으로 한다**(사용자 확정).
- 학습하기 → `/learn`, Quiz Start → `/quiz`
- Today: 오늘 일정 전체 노출 (앞으로 30일, 최대 3건)
- **자동재생** ✅ 구현 완료(2026-09-07): "학습하기" 버튼 옆 재생 아이콘 버튼으로 시작(항상 처음부터
  새로 재생만 하는 버튼 — 토글 아님, 일시정지/다음 전환은 미니 플레이어에서). 목록 전체를 재생
  대상으로 한다. 자세한 동작은 아래 "자동재생(공통)" 참고.
