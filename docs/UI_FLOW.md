...
- **자동재생** ✅ 구현 완료(2026-09-07): "학습하기" 버튼 옆 아이콘 토글(`▶`/`⏸`)로 시작. 켜면 현재
  슬라이드 단어를 읽고 1초 후 다음 슬라이드로 자동 이동. 재생 중일 때만 하단(BottomNav 바로 위)에
  미니 플레이어 바(`AutoPlayBar`, 단어+캡션+이전/재생·일시정지/다음)가 나타난다. 자세한 동작은 아래
  "자동재생(공통)" 참고.
...
- **자동재생** ✅ 구현 완료(2026-09-07): 목록 하단에 항상 노출되는 슬림 토글("자동재생"). 켜면 첫
  단어부터 순서대로 읽고, 1초 후 다음 단어로 진행하며 현재 카드가 화면 중앙으로 스크롤 + 테두리
  하이라이트된다. 재생 중엔 그 자리가 미니 플레이어 바(`AutoPlayBar`)로 바뀐다. 자세한 동작은 아래
  "자동재생(공통)" 참고.

---

### 자동재생(공통) — 홈 캐러셀 / 학습하기 목록

- 상태 모델(`useAutoPlay` 훅, `web/src/hooks/useAutoPlay.ts`): `active`(세션 시작 후 아직 안 끝남,
  미니 플레이어 노출 여부) / `playing`(현재 재생 중) / `index`. 엔트리 버튼과 미니 플레이어 바 중앙
  버튼은 물리적 위치만 다를 뿐 같은 토글 동작을 공유한다.
- 미니 플레이어 바(`web/src/components/autoplay/AutoPlayBar.tsx`): 어두운 배경, 현재 단어(굵게) +
  캡션(예문이 있으면 예문, 없으면 뜻) + 이전(⏮)/재생·일시정지(⏯)/다음(⏭) 버튼. 재생이 시작된
  순간에만 나타나고, 끝까지 재생하거나 다음 버튼을 마지막 단어에서 누르면 자동으로 사라진다(인덱스 0
  으로 리셋).
- **웹(브라우저)**: 백그라운드 보장 없이 `speechSynthesis` 완료 콜백 기반으로 훅이 직접 순차
  스케줄링한다. 탭이 보이는 동안만 정상 동작.
- **앱(RN 래퍼)**: 재생 시작 시 전체 단어 목록을 네이티브(`AUTOPLAY_START`)로 한 번에 전달하고,
  이후 순차 재생/타이머는 `mobile/App.tsx`의 RN JS 스레드가 전담해 화면 잠금/백그라운드에서도
  이어진다(iOS `UIBackgroundModes: audio` + Android 잠금화면 컨트롤용 무음 오디오 세션 유지, `expo-audio`).
  왜 웹뷰가 아니라 네이티브가 시퀀싱하는지는 `docs/DECISION_LOG.md` 2026-09-07 항목 참고.
