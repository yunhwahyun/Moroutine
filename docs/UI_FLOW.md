...
- **여러 날짜에 걸친 일정 표시** (2026-09-15 버그 수정, `docs/DECISION_LOG.md` 참고): 종일
  ...
  조회) 통째로 안 보였다. 수정 후 동작은 반복 여부에 따라 다르다:
  - **반복 없는 일정**: 걸치는 기간과 조회 범위가 겹치는 **날짜마다** 카드를 하나씩 만든다
    (실제 `starts_at`/`ends_at`은 그대로 유지, `occurrence_date`만 날짜별로 바뀜) — 당일
    일정(대다수)은 겹치는 날이 하루뿐이라 기존과 동일하게 1건만 나온다.
  - **반복 일정**: 회차당 카드 1장만 만들되(예전과 동일), 그 회차가 걸치는 기간이 조회 범위와
    겹치기만 하면 포함되도록 필터만 고쳤다 — 날짜별로 여러 장을 만들지 않는 이유는, 반복
    간격이 걸치는 기간보다 짧으면(2일짜리 종일 일정을 매일 반복 등) 서로 다른 회차가 같은
    표시 날짜를 공유해 `occurrence_id`가 충돌하고, 이게 삭제/수정이 엉뚱한 카드에 적용되는
    버그로 이어지기 때문(실사용자 리포트로 발견). 트레이드오프로, 시작일이 조회 범위보다 앞선
    회차는 그 시작일(범위 밖 날짜) 헤더 밑에 뜬다.
