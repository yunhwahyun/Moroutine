...
```
┌──────────────────────────────────┐  ← 배경색 A (학습영역)
│  [복습 단어 카드 1개]  +149        │  ← 첫 단어만 표시, 나머지는 카운트
│  [  학습하기  ] [▶]  [ Quiz Start ]│
│  (언어가 2개 이상 섞이면 이 블록이  │
│   언어별로 하나씩 더 반복된다)      │
└──────────────────────────────────┘
...
- 학습영역: 오늘 복습할 단어 목록(`useTodayStudyWords()`)을 단어장의 `language`별로 그룹
  나눈다(`groupWordsByLanguage()`) — 그룹마다 **첫 번째 단어 카드만** 표시하고 나머지는 "+N"
  카운트로, **학습하기/Quiz 시작/자동재생 시작 버튼도 그룹별로 따로** 둔다(2026-09-16 —
  서로 다른 언어의 단어를 한 세션에 섞으면 발음을 어느 언어로 읽어야 할지 애매해지는 문제,
  단어장 목록의 "다중 선택 시 언어 제한"과 같은 목적. "오늘의 복습"은 사용자가 직접 고르는
  게 아니라 전체 단어장에서 자동으로 모으므로 선택 자체를 막을 수 없어 대신 화면에서 분기하는
  방식을 택했다, 사용자 확정). 언어가 하나뿐이면 그룹도 하나라 그룹 라벨 없이 예전과 동일하게
  보인다. 카드 미리보기 자체는 "**첫 단어 하나만** 보여주고 나머지는 카운트로"인 것도
  2026-09-16 변경 — 예전엔 그룹 없이 전체를 가로 스와이프 슬라이드로 렌더링했는데, 복습할
  단어가 많으면(100개 이상) 카드가 전부 DOM에 올라가 스크롤이 버벅이고 멈추는 문제가 있었다
  (실사용자 리포트).
- 학습하기 → `/learn`, Quiz Start → `/quiz` (해당 그룹의 단어만 넘어감)
- Today: 오늘 일정 전체 노출 (앞으로 30일, 최대 3건)
- **자동재생** ✅ 구현 완료(2026-09-07): "학습하기" 버튼 옆 재생 아이콘 버튼으로 시작(항상 처음부터
  새로 재생만 하는 버튼 — 토글 아님, 일시정지/다음 전환은 미니 플레이어에서). 해당 그룹의 단어만
  재생 대상으로 한다. 자세한 동작은 아래 "자동재생(공통)" 참고.
...
- **다중 선택 시 언어 제한** ✅ 구현 완료(2026-09-16): 서로 다른 `language`의 단어장은 함께
  선택할 수 없다(발음 재생이 어느 언어를 따라야 할지 애매해지기 때문, 사용자 확정) — 이미
  선택된 것과 다른 언어의 단어장을 체크하려 하면 선택이 막히고 하단 액션바 위에 "언어가
  다른 단어장은 함께 선택할 수 없습니다" 메시지가 뜬다. 언어 미지정 단어장은 이 제약에서
  제외(어떤 언어와도 함께 선택 가능). 가상 항목 `'review'`("오늘의 복습")는 원래 여러 언어가
  섞이는 게 정상이라 이 제약 대상이 아니다(대신 홈 화면에서 언어별로 분기 — 아래 "홈" 참고).
- **일괄등록 버튼**: `permissions.canBulkImport`가 true인 등급(Master)에서만 노출, Guest는 버튼 자체를 숨김. 요금제 한도 개념은 2026-09-15 삭제 — 1차 출시는 결제가 없어(Guest/Master/Admin) 등록 개수 제한 자체가 없다(Guest는 로컬, Master는 서버로 저장 위치만 다를 뿐). 미리보기는 추가 예정/중복 제외/오류 행만 보여준다(`docs/DECISION_LOG.md` 2026-09-15)
...
- **해시태그 등록/필터** ✅ 구현 완료(2026-09-15, `docs/DECISION_LOG.md` 참고) — 개인 단어장 전용, 공용 단어장에는 없음:

  ```
  ┌──────────────────────────────────┐
  │  # 중1  # 아이엘츠  # 회화          │  ← 필터 칩 바 (등록된 태그 합집합, 없으면 숨김)
  ├──────────────────────────────────┤
  │  ☐  영어 단어장  #중1 #회화  단어 45개 │
  │  ☐  일본어 N3   #아이엘츠   단어 30개 │
  └──────────────────────────────────┘
  ...
  - **입력**: "+ 추가"/"✏ 수정" 폼에 이름 입력칸 아래 해시태그 입력칸 1개 추가(선택 입력, 비워도 됨).
    `placeholder="예) 중1, 아이엘츠, 회화"`. 쉼표(`,`)로 여러 개 입력.
  - **저장 시 정규화**: 쉼표로 분리 → 각 항목 앞뒤 공백 제거 → 맨 앞 `#` 있으면 제거 → 빈 문자열 제외 →
    완전히 같은 문자열끼리 중복 제거 → 태그 1개당 20자, 단어장당 최대 5개로 자름(초과분은 조용히 버림,
    별도 에러 없음). 저장은 원문 그대로(대소문자 변환/자모 정규화 없음).
  - **표시**: 목록 카드에 언어 뱃지 옆(또는 아래 줄)에 `#태그` 칩으로 나열. 태그 없는 단어장은 기존과
    동일하게 아무것도 표시 안 함.
  - **필터 칩 바**: 목록 최상단(헤더 바로 아래)에 현재 사용자의 전체 단어장에 등장한 해시태그를
    중복 없이 모아 칩으로 나열(정렬 기준: 등장 빈도 내림차순, 동률이면 가나다순). 태그가 하나도
    없으면 칩 바 자체를 숨긴다. 칩 탭 = 토글(다중 선택 가능) — 이미 화면에 로드된 목록(`getWordbooks()`
    전체 결과)에 대한 **클라이언트 사이드 필터**이며 서버 재조회 없음.
  - **필터 조건은 AND** — 칩을 여러 개 선택하면 선택한 태그를 **전부** 가진 단어장만 남는다(사용자 결정,
    2026-09-15 — 처음엔 OR로 설계했으나 태그가 늘수록 결과가 급격히 줄어드는 게 의도와 맞다고 판단해
    변경). 선택 칩 없음 = 필터 없음(전체 표시).
  - 필터 선택 상태는 화면을 벗어나면(다른 탭 이동, 새로고침) 초기화되는 휘발성 UI 상태 —
    영속 저장(로컬스토리지 등) 하지 않는다.
  - **필터 결과 없음 메시지 위치**: "선택한 태그를 모두 가진 단어장이 없습니다"는 목록 최상단이
    아니라 **복습 단어모음(가상 컬렉션) 카드 다음**에 노출한다(2026-09-15 사용자 확정 — 복습
    단어모음은 태그 필터와 무관하게 항상 보여야 하므로, 그 카드보다 위에 "결과 없음" 메시지가
    뜨면 복습 카드가 마치 가려진 것처럼 보여 혼란스러웠다).
  - **책장(`/books`)에도 동일하게 적용** — 아래 "책장" 절 참고, 폼/필터 UI는 단어장과 완전히 동일한
    패턴을 재사용한다(기존에도 "이름+언어" 폼이 `WordbookListPage`와 동일 컴포넌트 구조였던 것과 같은 이유).
    다만 책장에는 복습 단어모음 같은 가상 컬렉션이 없어 "결과 없음" 메시지 위치를 옮길 기준점이
    없다 — 필터 칩 바 바로 다음(원래 위치)에 그대로 둔다.
...
- **자동재생** ✅ 구현 완료(2026-09-07): 목록 하단에 항상 노출되는 버튼("자동재생", 토글 아님 —
  항상 첫 단어부터 새로 시작). 재생 중엔 현재 카드가 화면 중앙으로 스크롤 + 테두리 하이라이트된다.
  버튼 크기/스타일은 "학습 완료" 버튼과 동일한 크기의 라인(테두리)만 있는 버튼, 아이콘
  크기 24(2026-09-16 — 예전엔 더 작은 슬림 텍스트 버튼이었음). 자세한 동작은 아래
  "자동재생(공통)" 참고.
...
- **여러 날짜에 걸친 일정 표시** (2026-09-15 정리, `docs/DECISION_LOG.md` 참고): 종일 다중일자
  지원(§ 아래) 이후 발견 — `expandScheduleOccurrences`가 예전엔 occurrence를 **시작일 하나**에만
  붙여서, 시작일이 조회 범위보다 이전이면(예: 9/14~9/15 종일 일정을 "오늘"=9/15만 조회) 통째로
  안 보였다. 최종 설계는 **반복 여부와 무관하게 동일** — 일정(반복이면 그 회차)이 걸치는 기간과
  조회 범위가 겹치는 **날짜마다** 카드를 하나씩 만든다. `ScheduleOccurrence`에 필드 2개로 분리:
  - `occurrence_date` — 그 occurrence(회차)의 **진짜 시작일**. exceptions/"이후 모두 수정"
    분할의 자연키라 항상 고정, 여러 날짜에 걸쳐도 안 바뀐다.
  - `display_date` — 이 카드가 **어느 날짜 헤더 아래** 뜨는지. 여러 날짜에 걸치면 겹치는 날마다
    같은 `occurrence_date`를 공유하는 별도 엔트리가 생기고 이 값만 날짜별로 다르다.
  - `occurrence_id`(React key)는 `schedule_id:occurrence_date:display_date` 조합이라, 반복
    간격이 걸치는 기간보다 짧아 회차끼리 겹치는 극단적 설정(2일짜리 종일 일정을 매일 반복 등)
    에서도 항상 고유하다(회차마다 진짜 시작일이 다르므로) — 처음엔 이 충돌을 피하려고 반복
    일정만 회차당 카드 1장으로 되돌렸었는데, 이 키 설계로 충돌 자체를 없앤 뒤 반복 일정도
    하루짜리 일정과 동일하게 날짜마다 카드가 뜨도록 재정리(사용자가 "반복 일정만 표시가 다른
    게 의도된 거냐"고 물어 발견).
  - **카드 텍스트**(`formatScheduleCardTime()`, `web/src/lib/scheduleFormat.ts`): 종일 다중일자는
    시작일 카드에 `YY.MM.DD (요일) ~ YY.MM.DD (요일) 종일`(전체 범위), 그 이후 날짜의 카드에는
    `~ YY.MM.DD (요일) 종일`(계속 진행 중이라는 뜻)로 표시한다(사용자 확정). 당일 종일 일정은
    기존과 동일하게 그냥 `종일`.
  - `applyScheduleExceptions()`는 `occurrence_id`가 아니라 `schedule_id`+`occurrence_date`
    (진짜 식별자)로 매칭한다 — "이 일정만 취소/수정"이 여러 날짜에 걸친 회차의 카드 중 하나에서
    실행돼도 그 회차의 모든 표시 카드에 일관되게 적용된다.

**등록/수정 폼 — 날짜·시간 입력** (`ScheduleFormPanel`, 2026-09-15 정리):
- 날짜/시간 입력은 `NativeDateTimeInput`(`<input type="date"|"time">` 래퍼) — **아이콘 없이**
  브라우저 기본 값 표시만 사용한다. 예전엔 앱(WebView)에서만 네이티브 달력/시계 아이콘을 지우고
  자체 SVG 아이콘을 오버레이했는데, 실기기에서 화면이 깨져 보이는 문제가 있어 아이콘 자체를
  없앴다(사용자 확정). 시작/종료 각 행의 날짜·시간 입력 폭 비율은 **1:1**(예전 3:2 — 아이콘을
  없애면서 실제 텍스트 길이가 비슷해져 조정).
- **기본 시작 시간**: 새 일정 추가 시 현재 시각의 **분을 버린 정시**(13:20 → 13:00), **종료는
  항상 시작 + 1시간**. 시간 입력이 빈 값으로 뜨는 경우는 없다 — 기존 일정을 열었을 때도
  `ends_at`이 없으면(레거시 종일 데이터 등) 같은 규칙(시작 + 1시간)으로 채운다.
- **종일(`is_all_day`)**: 켜면 **시간 입력만** 화면에서 사라진다 — 시작/종료 날짜는 둘 다 계속
  보여서 여러 날에 걸친 종일 일정(예: 휴가)도 표현할 수 있다(2026-09-15 수정 — 처음엔 종료 행
  전체를 숨겨 종일 일정이 항상 하루짜리였는데, 종료 날짜는 계속 필요하다는 사용자 지적으로
  시간만 숨기도록 변경). 저장 값은 항상 `starts_at`=시작 날짜 `00:00`, `ends_at`=종료 날짜
  `23:59`(예전엔 `ends_at=null`로 저장해 "종일인데 종료 시각이 없다"는 상태였다 — 수정).
  입력 UI는 기본 체크박스가 아니라 `web/src/components/ui/Toggle.tsx`(검정/흰색 pill
  스위치, `SettingsPage.tsx`와 공용) — 사용자 확정.
- **알림 시간 셀렉트**(2026-09-16 추가): "알림 없음"/"정시"/"N분 전"(기존, `alarm_mode='offset'`
  + `alarm_minutes`)에 **"설정 알림 시간"**(`alarm_mode='daily_time'`) 옵션 추가 — 시작 시각과
  무관하게 설정 페이지("복습 알림"의 알림 시간, `reviewNotificationTime`)에 지정된 시각에
  알림이 온다. 종일 일정은 `starts_at`이 항상 자정이라 기존 오프셋 방식이 자정 근처에만
  알림을 줘서 쓸모없었던 문제를 해결하려고 추가했다(종일/시간 지정 일정 모두에 적용 가능한
  일반 옵션). 별도의 "일정 전용 알림 시간" 설정은 만들지 않고 설정 페이지 값을 그대로
  공유한다(사용자 확정, 설정 페이지 라벨도 변경 없음).
...
`DataRepository` 경유). 헤더에 **"+추가"**(이름+언어+해시태그, 단어장 추가 폼과 동일 —
해시태그 등록/필터 UI는 위 "단어장" 절 참고, `/public-books` 공용 책장에는 적용 안 함) 버튼과, Pro/Master
...
선택 순서 → 책 안에서는 목차 순서로 이어 붙여 순차 재생(랜덤 아님). 단어장과 동일하게 서로 다른
`language`의 책은 함께 선택할 수 없다(2026-09-16, 발음 재생 언어가 애매해지는 문제 방지 —
자세한 조건은 위 "단어장" 절의 "다중 선택 시 언어 제한" 참고). `/books/:id`(`BookDetailPage`)는
...
헤더에 노출). 제목/내용은 그 책의 `language` 설정에 맞는 음성으로 읽는다(2026-09-16 수정 — 예전엔
언어 설정과 무관하게 항상 영어 원음으로 읽어서 중국어/일본어 책장 발음이 안 되던 버그가 있었다).
학습하기·퀴즈·복습 진행률 추적은 전혀 없다 — 순수 읽기·듣기 콘텐츠.
...
목록을 읽기 전용으로 보여주고, 목차별 "듣기"는 개인 책장과 동일하게 동작하고 헤더에도 동일한
"전체 듣기" 버튼이 있다. 목차 내용 미리보기는 개인/관리자 책장과 동일하게
`ExpandableText`(기본 한 줄 말줄임 + "전체보기"/"한줄 보기" 토글 버튼)로 통일했다(2026-09-16
— 예전엔 제목 행을 탭하면 2줄 말줄임/전체 보기가 토글되는 별도 방식이었다).
