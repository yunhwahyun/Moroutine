...
- **해시태그 등록/필터** (설계, `docs/DECISION_LOG.md` 2026-09-15) — 개인 단어장 전용, 공용 단어장에는 없음:
...
- **종일(`is_all_day`)**: 켜면 시작 시간 입력과 종료 행(날짜+시간) 전체가 화면에서 사라지고
  시작 날짜만 남는다. 저장 값은 항상 `starts_at`=해당 날짜 `00:00`, `ends_at`=해당 날짜
  `23:59`(예전엔 `ends_at=null`로 저장해 "종일인데 종료 시각이 없다"는 상태였다 — 수정).
  종일 일정은 여러 날에 걸친 기간을 표현하지 않는다(종료 날짜 입력 자체가 없음, 하루 단위).
