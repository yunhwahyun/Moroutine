...
- Admin이 생성/수정, 사용자는 조회·학습만(제목/설명/단어 수정 불가, 원본 삭제 불가) — **원본 자체**에 대한
  정책이다. `PublicWordbookViewPage`의 미리보기 학습하기/퀴즈풀기는 이 원본을 참조 방식으로 읽는다.
- **"담기"는 복사 방식으로 전환(2026-09-02)**: 처음에는 "개인 단어장으로 자동 복제하지 않는다 — 원본
  참조 방식"이었으나, 사용자 요청에 따라 `PublicWordbookListPage.tsx`의 "내 단어장에 추가" 버튼은 이제
  공용 단어장의 단어를 사용자의 **개인** `wordbooks`/`words`로 실제 복사한다(`getRepository(tier)`의
  `createWordbook`/`bulkCreateWords` 재사용 — Pro 개인 단어 한도 검증도 이 경로를 그대로 타므로, 복사된
  단어는 **Pro 한도에 포함된다**, 아래 항목과 상충하지 않도록 주의). 복사 후에는 원본과 완전히
  분리된 개인 단어장이 되어 `WordbookDetailPage`에서 자유롭게 수정·삭제·단어 추가가 가능하다 — 이후
  관리자가 원본을 수정해도 이미 복사된 사본에는 반영되지 않는다(참조 방식과의 핵심 차이). "내 단어장에
  추가" 버튼은 이미 복사한 뒤에도 항상 다시 누를 수 있다(실수로 삭제했거나 다시 받고 싶은 경우 대비,
  2026-09-02 수정) — 이미 복사한 적이 있으면 제목 옆에 체크 배지만 표시하고, `user_public_wordbook_
  enrollments`에는 중복 삽입하지 않는다(마커 성격이라 재삽입할 필요가 없을 뿐, 버튼 자체를 막지는 않음).
  취소(담기 해제) 기능은 없다.
...
- 관리자가 단어 내용을 수정하면 **미리보기(학습하기/퀴즈풀기)에는** 즉시 반영된다(참조 방식이므로 자동
  충족) — 단, 이미 "담기"로 복사된 사용자의 개인 사본에는 반영되지 않는다(위 항목 참고).
- 원본(공용 단어) 자체는 Pro 개인 단어 한도에 포함하지 않는다(`docs/SUBSCRIPTION_DESIGN.md` §1) — "담기"로
  복사된 개인 단어는 일반 개인 단어와 동일하게 한도에 포함된다.
