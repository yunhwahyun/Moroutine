...
### 이용약관 조문 내 ①②③④ 항목이 한 줄로 붙어 보이던 문제(사용자 md 수정 후속)

**배경**: 사용자가 `web/public/legal/terms.md`/`docs/legal/TERMS_PHASE1.md`에서 각 조문의
①②③④ 항목을 줄마다 나눠 적도록 직접 수정(가독성 개선 의도). 그런데 커먼마크 기본 규칙상
문단 안의 개행 1개는 렌더링 시 공백으로 합쳐진다 — 소스에서만 줄이 나뉘고 화면에서는
다시 한 줄로 붙어 보인다(직접 렌더링해 확인, 사용자 의도와 다른 결과였을 것).

**수정**: `LegalDocumentPage.tsx`의 `p` 컴포넌트에 `whitespace-pre-line` 클래스 추가 —
문단 안의 단일 개행을 실제 줄바꿈으로 표시(문단 사이 빈 줄 구분은 그대로 유지).

**검증**: `whitespace-pre-line`을 붙인 상태로 직접 렌더링해 `\n`이 텍스트 노드에 남아있고
CSS로 줄바꿈이 적용되는 것 확인, 빌드 산출물 CSS에 해당 클래스가 실제로 포함됐는지 확인
(Tailwind 콘텐츠 스캔 대상). `tsc -b --force`/`eslint .`/`npm run build` 통과.

---

### 이용약관/개인정보처리방침 md의 HTML 주석이 화면에 그대로 노출되던 문제

**배경**: "md에 주석이 있을 경우 주석 코드 그대로 보여져" 리포트. `react-markdown`은
`allowDangerousHtml`을 켜지 않으면 raw HTML을 실행하지 않고 문자 그대로 이스케이프해서
찍는다 — H1 제목을 숨기려고 넣은 `<!-- # Moroutine ... -->` 주석이 화면에 그대로
노출됐다(react-dom/server로 직접 렌더링해 확인, 사용자 리포트와 일치).

**시도했다가 되돌린 방법**: `rehype-raw`를 붙이면 raw HTML을 실제로 파싱해 주석이 정상적으로
안 보이지만(직접 테스트로 확인), HTML 파서(parse5 등)를 통째로 끌고 와서 지연 로딩 청크가
gzip 48KB→101KB로 두 배 넘게 커졌다 — 이 문서에서 실제로 쓰는 HTML은 주석뿐이라 배보다
배꼽이 큰 선택이라 되돌렸다.

**수정**: `ReactMarkdown`에 넘기기 전에 `stripHtmlComments()`(정규식
`/<!--[\s\S]*?-->/g`)로 HTML 주석만 직접 제거 — 단일 줄/여러 줄 주석 둘 다 처리 확인.

**검증**: 실제 `public/legal/{terms,privacy-policy}.md` 파일로 정규식 결과 확인(주석 완전
제거), `tsc -b --force`/`eslint .`/`npm run build` 통과 + 청크 크기가 되돌린 것 확인(gzip
48KB, `rehype-raw` 적용 전과 동일).

---

### 이용약관/개인정보처리방침 화면 — 마크다운 실제 렌더링 + 지연 로딩

**배경**: "md파일을 로드하는거야??? 화면에 md 뷰어가 아니라서 이상하게 나오는데" 리포트.
`LegalDocumentPage.tsx`(`/terms`, `/privacy`가 공유하는 뷰어)는 마크다운 렌더러 없이
`fetch()`한 원문을 `<pre>`에 그대로 찍고 있었다 — `**굵게**`, `#제목`, 표 구분선(`|`,
`---`) 같은 마크다운 기호가 그대로 화면에 보였다. 원래 이렇게 만든 이유(주석에 남아있던
설명)는 "게시 전 `[확인 필요]` 표시를 임의로 안 지우기 위해"였는데, 이는 렌더러를 안 쓰는
것과는 별개 문제였다 — 마크다운을 실제로 렌더링해도 `` `[확인 필요]` `` 백틱 부분은
인라인 코드 스타일로 그대로 남아 오히려 더 눈에 띈다.

**수정**: `react-markdown` + `remark-gfm`(표 지원) 추가, `LegalDocumentPage.tsx`가
`<pre>` 대신 실제 마크다운을 렌더링하도록 변경(제목/굵게/표/링크/목록/인용/인라인 코드에
이 앱 톤에 맞는 최소한의 Tailwind 클래스 매핑).

**성능 고려**: 이 두 패키지가 gzip 기준 약 48KB로 꽤 무거워서(이 앱 전체 라우트 중
유일하게) `PrivacyPolicyPage`/`TermsPage`만 `React.lazy()` + `Suspense`로 지연 로딩하도록
`routes/index.tsx`를 바꿨다 — 이 앱은 지금까지 코드 스플리팅을 전혀 안 쓰고 있었지만, 자주
안 들어가는 화면이고 `LegalDocumentPage`가 어차피 `fetch(src)` 응답을 기다리는 동안 항상
Spinner를 보여주고 있어서 Suspense fallback과 자연스럽게 이어진다고 판단했다. 빌드
결과: 지연 로딩 전 메인 청크 930KB(gzip 265KB) → 지연 로딩 후 771KB(gzip 217KB, 기존과
거의 동일) + 별도 청크 `LegalDocumentPage` 160KB(gzip 48KB, `/terms`·`/privacy` 방문 시에만
로드).

**검증**: `tsc -b --force`/`eslint .`/`npm run build` 통과, `vite build` 산출물에
...
남겨둔 범위 밖 항목 — 서로 다른 언어의 단어장/책장을 "오늘의 복습"이나 목록에서 여러 개
선택해 하나의 학습하기/퀴즈/자동재생 세션으로 묶으면 어떤 언어를 기준으로 읽어야 할지
애매하다는 문제. 사용자 확정: "오늘의 복습은 언어별로 분기하고, 여러개 선택은 여러 언어를
동시에 선택 할 수 없게하고, 선택시 메세지를 띄우는걸로 하면 좋겠어".

**수정 1 — 다중 선택 시 다른 언어 차단**(`WordbookListPage.tsx`/`BookshelfListPage.tsx`):
`toggleId()`에서 새로 선택하려는 항목의 `language`가 이미 선택된 항목들의 `language`와
다르면 선택 자체를 막고 하단 액션바 위에 인라인 에러 메시지("언어가 다른 단어장은 함께
선택할 수 없습니다")를 띄운다. 언어 미지정(`null`) 항목은 판정에서 제외(막지 않음).
`WordbookListPage.tsx`의 가상 항목 `'review'`("오늘의 복습")는 원래 여러 언어가 섞이는 게
정상이라 이 제약에서 제외.

**수정 2 — "오늘의 복습" 언어별 분기**(`HomePage.tsx`): 다중 선택은 위에서 아예 언어를
섞을 수 없게 막았지만, "오늘의 복습"은 사용자가 고를 수 없이 전체 단어장의 복습 대상을
자동으로 모으므로 같은 방식으로 막을 수 없다 — 대신 화면에서 언어별로 그룹을 나눠(`
groupWordsByLanguage()`, `wordbook_id → language` 매핑) 그룹마다 독립된 미리보기 카드 +
학습하기/자동재생/Quiz 시작 버튼을 갖게 했다(`ReviewGroupBlock`). 언어가 하나뿐이면 그룹도
하나라 그룹 라벨을 안 보여주고 예전과 동일하게 보인다(사용자 대다수 케이스, 회귀 없음).

**신규**: `sourceLangLabel()`(`src/lib/ttsLang.ts`) — `language` 값을 화면 표시용 한글
라벨(영어/일본어/중국어/기타)로 변환. `sourceTTSLang()`과 동일한 값 집합을 다루지만
용도가 달라(재생 언어 코드 vs 화면 라벨) 폴백값도 다르다(`en-US` vs `기타`) — 재생
쪽은 레거시 콘텐츠를 영어로 취급해 소리는 나게 하는 게 우선이고, 라벨 쪽은 여러 그룹을
구분해 보여주는 용도라 "기타"가 더 정확하다.

**검증**: 다중 선택 차단 로직과 언어별 그룹핑 로직 둘 다 `tsx` 스크립트로 케이스별
(같은 언어/다른 언어/언어 미지정/`review`) 직접 확인. `tsc -b --force`/`eslint .`/
`npm run build` 통과. 웹 전용 수정.
...
"수정완료 | 취소" 한 줄 아래에 별도로 "이 단어 삭제"(전체 너비, 얇은 텍스트) 버튼이 있고,
책장 목차 수정 폼(`BookDetailPage.tsx`)은 "수정완료 | 삭제 | 취소"가 한 줄에 있는 다른
스타일이었다.

**수정**: `WordbookDetailPage.tsx`의 `FormActions`에 선택적 `onDelete`/`isDeleting` prop을
추가해 "삭제"를 저장/취소 사이 버튼으로 합쳤다(`BookDetailPage.tsx` 챕터 수정 폼과 동일한
버튼 3개 배치·스타일: 저장은 `flex-1` 채움 버튼, 삭제는 `border-red-200`, 취소는
`flex-1` 아웃라인). 새 단어 추가 폼은 `onDelete`를 안 넘겨서 기존처럼 "추가 | 취소" 2개만
남는다(책장의 "새 목차 추가" 폼과 동일한 패턴).

### 일정 알림 관련 실사용자 리포트 3건 — 설정 미반영 / 중복 알림 / 안드로이드 지연

**리포트 1 — "설정 알림 시간"을 쓰는 일정 등록 후 설정에서 알림 시간을 바꿔도 반영 안 됨**:
확인 결과 실제로 버그였다. `refreshScheduleNotifications()`는 일정 생성/수정 시점에만
호출돼서, 이미 등록된 daily_time 일정은 설정 페이지의 `reviewNotificationTime`이 바뀌어도
다시 계산되지 않았다. `ReviewNotificationSync.tsx`와 동일한 패턴으로
`ScheduleDailyTimeSync.tsx`를 신설 — `reviewNotificationTime`이 바뀔 때마다
`alarm_mode='daily_time'`인 일정 전체를 새 시각으로 다시 예약한다
(`refreshDailyTimeScheduleNotifications()`, `notificationScheduler.ts`).

**리포트 2 — 알림 1개를 기대했는데 두 번 연달아 옴**: 코드 리뷰로 실재하는 레이스 컨디션을
발견했다. `bridge.scheduleNotification()`이 지금까지 fire-and-forget이라(네이티브 응답을
기다리지 않고 즉시 반환), `refreshScheduleNotifications()`가 새 알림을 예약하고 반환한
직후 같은 일정이 다시 수정되면(`updateSchedule` 재호출 등) — 방금 예약한 알림의
`native_id`가 아직 DB에 저장되기 전이라 `cancelScheduleNotifications()`가 그 네이티브
알람을 못 찾아 취소하지 못한 채 넘어간다. 취소되지 않은 옛 알람 + 새로 예약된 알람이 같은
일정에 겹쳐 남아 중복으로 울릴 수 있다. `bridge.scheduleNotification()`을 네이티브의
`NOTIFICATION_RESULT` 응답(또는 8초 타임아웃)까지 기다리는 Promise로 바꾸고,
`refreshScheduleNotifications()`가 `native_id` 저장까지 끝난 뒤에 반환하도록 수정해
이 레이스 윈도우를 닫았다. 웹 배포만으로 적용되며 모바일 앱 재빌드는 필요 없다(브릿지
...
"여러 날짜 걸친 일정 표시 재설계"에서 다중일자 일정(반복 없음)이 겹치는 날짜마다 occurrence를
하나씩 만들도록 바꿨는데, `notificationScheduler.ts`의 `refreshScheduleNotifications()`가 이
occurrence 목록을 그대로 순회해서 **같은 회차의 알림을 겹치는 날짜 수만큼 중복 예약**하고
있었다(3일짜리 종일 일정이면 알림 3개가 전부 같은 시각에 잡힘 — `tsx`로 직접 재현·확인).

**수정**: `occurrence_date`(회차의 진짜 식별자, 여러 날짜에 걸쳐도 안 바뀜) 기준으로 중복
제거한 뒤 알림을 계산하도록 수정 — `applyScheduleExceptions()`에서 이미 쓰던 것과 동일한
식별 원칙. 반복 일정(회차마다 `occurrence_date`가 다름)은 회귀 없이 그대로 각 회차마다 알림이
잡힌다.

**검증**: `tsx`로 3일짜리 종일 일정(dedup 후 알림 1개) / 매일 반복 3회 당일 일정(dedup 후
3개 그대로) 직접 확인, `tsc --noEmit`/`npm run build`/`eslint` 통과. **한계**: 브라우저
자동화 도구가 없어 실제 기기에서 네이티브 알림이 정확히 1번만 오는지는 검증 못함.

---

### 반복 종료 "날짜까지" 입력이 아이폰에서 값 없이 작게 렌더링되던 문제

**배경**: 일정 반복 종료 방식을 "날짜까지"로 고르면 `repeatUntil` 날짜 입력이 뜨는데, 처음
고르는 순간엔 값이 비어있다(`defaultForm()`의 `repeatUntil: ''`). 아이폰 Safari는 값이 없는
`<input type="date">`를 다른 입력들보다 훨씬 작게 그린다는 사용자 리포트 — 값을 채우면 정상
크기로 나온다고 확인.

**결정**: 사용자가 제안한 두 가지를 모두 적용.
- **주 대책**: "횟수 반복"/"종료 없음" → "날짜까지"로 바꾸는 순간, `repeatUntil`이 비어있으면
  일정 시작일(`form.date`)을 기본값으로 채운다 — 항상 유효한 값이 있는 상태로 렌더링되므로
  아이폰의 "빈 값이면 작게 그리는" 동작 자체를 피해간다. 이미 값이 있으면 덮어쓰지 않는다.
- **보조 대책**: `index.css`의 입력 최소 높이 규칙(`min-height: 2.75rem`)에 `date`/`time`도
  포함시켰다 — 값이 있든 없든 다른 입력과 높이가 안 맞을 가능성 자체를 줄인다. 다만 이
  프로젝트는 과거에 iOS의 date/time input이 CSS 너비 지정을 무시하는 사례를 이미
  겪었어서(`docs/DECISION_LOG.md` 2026-09-08), 높이 쪽도 완전히 먹히지 않을 가능성을 염두에
  두고 주 대책(값 채우기)을 우선했다.

**검증**: `tsc --noEmit`/`npm run build`/`eslint` 통과. **한계**: 브라우저 자동화 도구가 없어
실제 아이폰에서 크기가 정상으로 보이는지는 검증 못함 — 사용자 확인 필요.

---

### 개인 단어장/책장 상세 헤더 — 제목 줄/버튼 줄 사이 구분선 추가

**배경**: 바로 앞 항목("제목 줄과 버튼 줄 분리")에서 헤더를 두 줄로 나눴는데, 두 줄 사이에
아무 구분선이 없어 어색하다는 후속 지적. 그냥 `border-t`만 추가하면 제목 바로 아래에 선이
붙어버려 답답해 보인다는 점도 함께 지적 — 제목 위쪽 여백(헤더 컨테이너의
`paddingTop: 0.75rem`)만큼 버튼 영역 위쪽에도 여백을 준 다음 그 자리에 선을 그어야 균형이
맞는다.

**결정**: 버튼 줄 컨테이너에 `mt-3`(0.75rem, 제목 위 여백과 동일) + `border-t border-gray-100`
(구분선) + 기존 `pt-2`(선과 버튼 사이 여백)를 함께 적용 — `WordbookDetailPage.tsx`/
`BookDetailPage.tsx` 양쪽 동일하게. 헤더 전체를 감싸는 `border-b border-gray-100`(헤더와
본문 사이 경계)과는 별개 용도라 겹쳐도 어색하지 않다.

**검증**: `tsc --noEmit`/`npm run build`/`eslint` 통과. **한계**: 브라우저 자동화 도구가
없어 실제 화면(여백 간격이 의도한 대로 보이는지)은 검증 못함.

---

### "듣기"/"전체 듣기" 버튼에 테두리 추가

**배경**: `BookDetailPage.tsx`(개인 책장)/`PublicBookViewPage.tsx`(공용 책장 열람)의 "듣기"/
"전체 듣기" 버튼이 테두리도 배경도 없이 재생 아이콘만 덩그러니 떠 있어 버튼처럼 안 보이고
어색하다는 지적("arrow만 있어서 어색해") — 재생 아이콘(▶)이 시각적으로 화살표처럼 보이는데
주변에 아무 경계가 없으니 눌러야 할 버튼이라는 신호가 약했다.

**결정**: 두 화면의 헤더 "전체 듣기"와 목차별 "듣기" 버튼에 `border border-gray-200` 테두리를
추가 — 헤더의 다른 버튼(일괄등록 등)이나 목차 카드의 다른 아이콘 버튼과 달리 유독 이 버튼만
테두리가 없었던 걸 맞췄다. 같은 줄의 "수정" 아이콘 버튼은 앱 전반에서 원래도 테두리 없는
플레인 아이콘 패턴이라 그대로 둠(사용자가 "듣기/전체듣기"만 지목).
`PublicBookViewPage.tsx`의 목차별 듣기 버튼은 기존 `rounded-full`을 다른 화면과 통일해
`rounded-md`로 맞췄다.

**검증**: `tsc --noEmit`/`npm run build`/`eslint` 통과. **한계**: 브라우저 자동화 도구가
없어 실제 화면(테두리가 자연스러워 보이는지)은 검증 못함.

---

### 책장 목차 내용 미리보기 — 관리자/개인 화면 표시 방식 통일 + 전체보기/한줄 보기 토글

**배경**: 목차 내용(`chapter.content`) 미리보기가 화면마다 달랐다 — 관리자
(`AdminBookDetailPage.tsx`)는 `line-clamp-3`(3줄 말줄임, 펼치기 불가), 개인
(`BookDetailPage.tsx`)은 아예 잘리지 않고 전체 노출 — 목차가 길면 카드가 한없이 늘어났다.
사용자가 "노출이 통일돼야 할 것 같다"고 지적.

**결정**: 공용 컴포넌트 `web/src/components/ui/ExpandableText.tsx` 신설 — 기본은 **한 줄
말줄임**(Tailwind `truncate`), "전체보기" 버튼을 누르면 원문 그대로(`whitespace-pre-wrap`,
줄바꿈 보존) 펼쳐지고 버튼이 "한줄 보기"로 바뀌어 다시 접을 수 있다. 두 화면 모두 이 컴포넌트
하나로 교체 — 관리자/개인 책장 상세의 목차 미리보기가 이제 완전히 동일하게 동작한다. 내용이
실제로 잘리는지 여부와 무관하게 내용이 있으면 항상 토글 버튼을 보여준다(정확한 오버플로우
감지는 DOM 측정이 필요해 과한 엔지니어링으로 판단, 사용자 요구는 "토글 가능"이지 "필요할 때만
보이기"가 아니었음).

**검증**: `tsc --noEmit`/`npm run build`(불필요해진 `line-clamp-3` 관련 CSS가 빠져 번들 용량도
약간 감소)/`eslint` 통과. **한계**: 브라우저 자동화 도구가 없어 실제 토글 클릭 동작은 검증 못함.

---

### 개인 단어장/책장 상세 헤더 — 제목 줄과 버튼 줄 분리

**배경**: `WordbookDetailPage.tsx`/`BookDetailPage.tsx`(사용자가 직접 만드는 개인 단어장/책장
상세, 관리자 화면 아님) 헤더가 뒤로가기·제목·버튼(최대 3~4개: 일괄등록/+추가/비우기, 책장은
전체 듣기까지 4개)을 한 줄에 다 넣고 있었다 — 버튼이 최대치로 나오면 제목이 `max-w-[160px]`
안에서 거의 안 보일 정도로 눌렸다(사용자 확정).

**결정**: 헤더를 두 줄로 분리 — 첫 줄은 뒤로가기 + 제목(이제 `flex-1 min-w-0`으로 남는 폭을
전부 사용, `max-w-[160px]` 제거), 둘째 줄은 기존 버튼들을 오른쪽 정렬로 배치(`flex-wrap`도
추가해 아주 좁은 화면에서 버튼이 넘치면 줄바꿈되게). 관리자 단어장/책장 상세(`AdminWordbookDetailPage.tsx`/
`AdminBookDetailPage.tsx`)는 바로 위 항목에서 헤더 버튼을 "일괄등록" 하나로 줄여놔서 이 문제가
없어 대상에서 제외했다(사용자가 "사용자 단어장/책장"이라고 명시적으로 범위를 좁힘).

**검증**: `tsc --noEmit`/`npm run build`/`eslint` 통과. **한계**: 브라우저 자동화 도구가 없어
실제 화면(제목 가독성, 좁은 화면에서 버튼 줄바꿈)은 검증 못함.

---

### 관리자 단어장/책장 상세 — 헤더의 "삭제" 버튼을 메타 수정 폼으로 이동

**배경**: `AdminWordbookDetailPage.tsx`/`AdminBookDetailPage.tsx` 헤더에 "일괄등록" 버튼
바로 옆에 "삭제" 버튼이 있었는데, 사용자가 이게 단어장(책) 전체를 삭제하는 건지 다른 걸
삭제하는 건지 애매하다고 지적 — 실제로 아래쪽 메타 수정 폼에 "비우기"(단어/목차만 전부 삭제,
단어장/책 자체는 유지)라는 비슷한 버튼이 따로 있어서 두 삭제 액션이 화면 위/아래로 떨어져
있는 게 혼란의 원인이었다.

**결정**: 헤더의 "삭제" 버튼을 제거하고, 메타 수정 폼의 "비우기" 버튼 옆으로 옮겼다. 라벨도
"삭제"/"비우기"처럼 대상이 안 드러나는 이름 대신 **"단어장 삭제"/"단어 비우기"**(책은
"책 삭제"/"목차 비우기")로 명시해 범위를 이름만 보고도 구분할 수 있게 했다. 레이아웃은
"저장" 버튼을 폼 전체 너비로 단독 배치하고, 그 아래 "비우기"/"삭제" 두 개를 나란히
배치(둘 다 파괴적 액션이라는 걸 시각적으로도 묶어서 보여줌).

**검증**: `tsc --noEmit`/`npm run build`/`eslint` 통과. **한계**: 브라우저 자동화 도구가
없어 실제 화면 레이아웃(좁은 화면에서 버튼 줄바꿈 등)은 검증 못함.
...
**원인**: 바로 위 항목에서 "종일이어도 종료 날짜는 계속 노출"하도록 바꾸면서 종일 일정이
처음으로 여러 날짜에 걸칠 수 있게 됐는데, `scheduleRepeat.ts`의 `expandScheduleOccurrences()`는
occurrence를 항상 **시작일 하루에만** 붙이고 그 시작일이 조회 범위([fromDate, toDate]) 안에
있는지만 확인했다. 그래서 9/14~9/15 종일 일정을 만들고 "오늘"(=9/15, 오늘 이후로만 넓어지는
프리셋이라 어차피 과거는 절대 안 보임)을 조회하면 시작일(9/14)이 범위보다 이전이라 걸려서
전체 occurrence가 통째로 빠졌다 — 실제로는 9/15에도 그 일정이 "진행 중"인데도. 이 결함 자체는
"자정을 넘기는 시간 일정"(all-day 아닌 일정도 종료 날짜가 다음날일 수 있음)에도 원래 있었지만,
종일 일정은 지금까지 항상 하루짜리로 강제돼 있어 드러난 적이 없었다.

**수정(1차)**: `expandScheduleOccurrences()`가 "일정이 걸치는 기간"과 "조회 범위"가 **겹치는
날짜마다** occurrence를 하나씩 만들도록 변경(`pushOverlappingDays()` 신설, 반복 일정 포함
전체 케이스에 적용). 실제 `starts_at`/`ends_at` 값은 그대로 유지하고, 카드가 어느 날짜 헤더
아래 붙을지(`occurrence_date`)만 날짜별로 바뀐다.

**후속 수정(같은 날, 사용자가 "그 이벤트만/이후 모두 삭제도 확인해달라"고 요청해 발견)**: 1차
수정을 **반복 일정**에도 그대로 적용했더니, 반복 간격이 일정 기간보다 짧은 설정(예: 2일짜리
종일 일정을 "매일" 반복)에서 서로 다른 회차가 같은 표시 날짜를 공유해 `occurrence_id`가
충돌했다(예: 9/14~9/15 종일 + 매일 반복 2회를 "오늘"(9/15)에서 조회하면 두 회차 모두 9/15에
카드를 만들어 `occurrence_id`가 똑같아짐) — React key 충돌로 삭제/수정이 엉뚱한 카드에
적용되거나 화면이 갱신되지 않는 것처럼 보일 수 있었다("일정 삭제가 안 되는데?" 리포트로 이어질
수 있는 사례). **반복 일정은 회차당 카드 1장만 만들도록 되돌리고**(예전과 동일, 회차별
`occurrence_date`는 항상 서로 다른 실제 시작일이라 충돌 불가능), 그 회차가 걸치는 기간이 조회
범위와 겹치기만 하면 포함시키는 필터만 남겼다 — 대신 시작일이 조회 범위보다 앞선 회차는 그
시작일(범위 밖 날짜) 헤더 밑에 뜬다(사소한 트레이드오프, 안전성을 우선). **반복 없는 일정은
그대로 날짜별 여러 장** 유지(회차 개념이 없어 충돌 위험 자체가 없음).

같은 점검 과정에서 진짜 버그 하나 더 발견: 반복 일정 "이후 모두 수정"(`splitAndUpdate`)이
새로 쪼개는 schedule의 `endDate`를 항상 `date`(=선택한 회차의 시작일)로 강제하고 있어서, 여러
날짜에 걸친 반복 일정을 "이후 모두 수정"하면 새 schedule의 기간이 하루로 뭉개졌다(예전엔 반복
일정이 항상 하루짜리였어서 드러난 적 없던 결함). 사용자가 폼에서 실제로 설정한 시작~종료
간격(`f.date`~`f.endDate`)을 그대로 유지한 채 시작일만 옮기도록 수정.

**검증**: `tsx`로 시나리오 10개 이상 직접 실행 확인 — 당일 반복 회귀, 범위 밖 제외 회귀,
다중일자 종일 노출(반복 없음/있음 둘 다), 반복 다중일자 occurrence_id 충돌 없음, "이후 모두
삭제" cutDate 계산(중간 회차 선택 시 truncate / 첫 회차 선택 시 전체삭제 분기), "이후 모두 수정"
endDate 보존 계산. `tsc --noEmit`/`npm run build`/`eslint` 통과.

**한계**: 브라우저 자동화 도구가 없어 실제 화면에서 클릭 조작(삭제 확인 다이얼로그, 반복 범위
선택 모달)까지는 재현·재확인 못함 — 순수 함수 레벨(occurrence 계산/필터/날짜 산술)만 검증.

**상세**: `docs/UI_FLOW.md` "일정(`/schedules`)" 참고.
...
Moroutine 화면 1장)을 첨부하며 "화면이 깨지고 있다"고 지적 — 실제 화면에는 날짜/시간 입력
옆에 커스텀 캘린더·시계 아이콘이 붙어 있는데, 참고 이미지처럼 아이콘 없이 깔끔한 텍스트만
있는 모습을 원함.

**결정**:
- **아이콘 완전 제거** — `NativeDateTimeInput.tsx`가 앱(WebView)에서만 네이티브 아이콘을
  지우고 `CalendarIcon`/`ClockIcon`을 오버레이하던 로직을 삭제. 이제 앱/웹 모두 아이콘 없이
  텍스트 값만 노출(네이티브 달력/시계 픽커 아이콘 자체는 여전히 CSS로 숨김 —
  `.native-datetime-input::-webkit-calendar-picker-indicator`, 안 그러면 removed 커스텀
  아이콘 자리에 브라우저 기본 아이콘이 다시 나타남).
- **날짜:시간 폭 비율을 3:2 → 1:1로 변경**(`ScheduleFormPanel`) — 아이콘 제거로 아이콘용
  여백(`pr-5`/`paddingRight`)도 함께 없앴고, 실제 텍스트 길이("2026. 9. 15." vs "오후 1:00")가
  비슷해 균등 비율이 더 자연스러움.
- **기본 시작 시간 계산 방식 변경** — 예전엔 현재 분을 15분 단위로 올림(예: 13:20→13:30)했는데,
  "현재 시각 기준으로 13:20이면 13:00으로" 요청에 따라 **분을 그냥 버리는 정시**로 변경. 종료는
  그대로 시작 + 1시간 유지.
- **종일 저장값 수정(실질 버그)** — `is_all_day=true`일 때 `ends_at`을 그동안 `null`로
  저장했는데, "시작 00:00, 종료 23:59로 저장해달라"는 요청에 따라 `${date}T23:59:00`로 저장하도록
  수정. 겸사겸사 발견: `ends_at`이 없는 occurrence를 편집 폼에 불러올 때 `endTime`이 빈 문자열로
  세팅되던 버그(레거시 종일 데이터, 또는 수정 전 코드로 만들어진 종일 일정을 열었을 때 재현)도
  함께 고쳐 항상 "시작 + 1시간"으로 채우게 했다 — "시간은 빈 값이 없이" 요청과 직접 관련된
  케이스라 같이 처리.

**상세**: `docs/UI_FLOW.md` "일정(`/schedules`)" §등록/수정 폼 — 날짜·시간 입력.
**검증**: `tsc --noEmit`/`npm run build`/`eslint` 통과. **한계**: 브라우저 자동화 도구가 없어
실제 화면(아이콘 사라짐, 종일 토글, 기본 시간값)은 검증 못함 — 스크린샷으로 재확인 필요.

**후속(같은 날, 사용자 확인 후 진행)**: "종일" 입력도 참고 이미지처럼 토글 스위치 모양으로
바꿔달라는 요청 — 기본 `<input type="checkbox">`가 나머지 폼(알약 모양 pill 입력들)과
스타일이 안 맞아 "화면이 깨져 보인다"는 지적의 일부였다. `SettingsPage.tsx`에만 로컬로 있던
`Toggle` 컴포넌트(검정/흰색 pill 스위치, 이미 앱 전역에서 쓰이는 디자인)를
`web/src/components/ui/Toggle.tsx` 공용 컴포넌트로 추출해 `SettingsPage.tsx`/
`ScheduleListPage.tsx` 양쪽에서 재사용("공용 컴포넌트는 `src/components/`에서 import" 규칙,
`docs/DEVELOPMENT_RULES.md`). 새 디자인 시스템을 만들지 않고 기존 걸 재사용한 것.

---

### 단어 일괄등록 — 쉼표(.csv)/탭(.tsv/.txt) 구분자 모두 지원

**배경**: 사용자·관리자 양쪽 단어 일괄등록이 `.txt` 파일 + 탭 구분만 지원했는데, 사용자가
쉼표 구분 `.csv`와 `.tsv`도 지원해달라고 요청 — 스프레드시트(Excel/구글시트)에서 내보낸
CSV를 그대로 붙여넣고 싶은 수요.

**결정**: 구분자는 **파일 확장자로 결정**한다 — `.csv`→쉼표, 그 외(`.tsv`/`.txt`)→탭(기존
`.txt` 동작 그대로 유지, 회귀 없음). 내용 스니핑으로 구분자를 추측하는 방식은 채택하지
않았다(예측 불가능한 결과보다 확장자 기반이 사용자에게 더 명확).

- 따옴표(`"`)로 감싼 필드 안의 구분자·줄바꿈·이스케이프된 큰따옴표(`""`)를 표준 CSV 규칙대로
  처리하는 파서를 새로 작성 — 정의/예문에 쉼표나 실제 줄바꿈이 들어있는 CSV 내보내기 파일도
  그대로 붙여넣을 수 있다. 기존의 리터럴 `\n` → 실제 줄바꿈 치환 관례(따옴표 없이 한 줄
  형식이던 구 `.txt`용 우회)도 하위 호환을 위해 그대로 유지.
- 개인 단어장(`WordbookDetailPage.tsx`)/공용 단어장 관리자(`AdminWordbookDetailPage.tsx`)에
  각각 중복 구현돼 있던 `parseWordsTxt()`를 `web/src/lib/bulkWordsParse.ts`의
  `parseWordsFile(filename, content)` 공용 유틸로 통합.
- 파일 입력 `accept`를 `.txt` → `.csv,.tsv,.txt`로 확장. Admin 버튼 라벨 `.txt 일괄등록` →
  `일괄등록`(형식 중립적으로 변경).
- **헤더 행 자동 스킵은 넣지 않았다** — "단어,뜻,예문" 같은 헤더 줄도 그대로 단어 1개로
  등록된다(휴리스틱 판별의 오탐 위험 > 이득으로 판단, 기존 `.txt`도 애초에 헤더 개념이 없었음).
  업로드 전 헤더 줄 제거는 사용자 책임.

**상세**: `docs/ADMIN_DESIGN.md` §8-4, `docs/DB_SCHEMA.md`(`words.example` 컬럼 주석).

**RFC 4180 재검토(2026-09-15, 사용자 요청)**: "콘텐츠 안에 쉼표가 있을 때" 실제 CSV 표준
문서(필드 내 쉼표/줄바꿈/큰따옴표가 있으면 전체를 큰따옴표로 감싸고, 필드 내 큰따옴표는 `""`로
이스케이프)를 다시 확인하고 아래 케이스를 `tsx`로 직접 실행해 검증:
- 따옴표로 감싼 필드 안의 쉼표 → 구분자로 오인하지 않고 한 필드로 유지 ✅
- 따옴표 안의 실제 줄바꿈(여러 줄짜리 정의) → 행 경계로 오인하지 않음 ✅
- 이스케이프된 큰따옴표(`""`) → 리터럴 `"` 하나로 복원 ✅
- 탭 구분(.tsv/.txt)은 쉼표를 구분자로 취급하지 않아 원래부터 영향 없음(회귀 없음) ✅

이 과정에서 **실제 버그 1건 발견·수정**: 따옴표로 감싼 필드 안에 Windows 줄바꿈(CRLF)이
포함된 여러 줄짜리 셀(엑셀 등에서 흔함)을 붙여넣으면, 기존 코드가 큰따옴표 밖에서만 `\r`을
제거하고 있어서 따옴표 **안**의 `\r`이 정의/예문 텍스트에 그대로 섞여 들어가는 문제가 있었다.
`\r` 스킵 처리를 따옴표 안팎 모두에 적용하도록 수정(`parseDelimitedRows()`의 CR 정규화를
루프 최상단으로 이동). 수정 전/후를 비교하는 회귀 테스트 케이스 7개(csv 단순/쉼표 포함/
CRLF 포함 줄바꿈/이스케이프 따옴표, tsv 회귀, txt 레거시 `\n` 치환, 빈 줄·오류 행 카운트)를
모두 통과 확인.

**알려진 한계(의도적으로 안 고침)**: 따옴표가 홀수 개로 닫히지 않는 등 진짜로 깨진 CSV는
표준적으로도 정의가 불가능한 입력이라 이후 줄들을 하나의 필드로 흡수해버릴 수 있다 — 스프레드
시트 프로그램에 잘못된 CSV를 열어도 비슷하게 깨지는 것과 동일한 근본 한계이며, 별도 방어
로직(예: 따옴표 짝 맞음 검증 후 에러 처리)은 이번 범위에서 추가하지 않았다.

**검증**: `tsx`로 파서 단독 실행해 탭/쉼표/따옴표-포함 CSV/빈 줄/CRLF 케이스 확인,
`tsc --noEmit`/`npm run build`/`eslint` 통과. **한계**: 브라우저 자동화 도구가 없어 실제
파일 업로드 UI 동작(미리보기 화면)은 검증 못함.
...
- **GuestMigrationGate가 `/reset-password`에서도 뜨는 버그**: 이 컴포넌트는 경로 확인 없이
  `serviceTier`만 보고 전역으로 뜨는데, `DowngradeGate`만 `EXEMPT_PATHS`로 이미 막아뒀고
  `GuestMigrationGate`는 빠져 있었다(둘 다 "인증됨 + tier"만 보는 동일 계열 전역 모달인데
  하나만 고쳐져 있던 상태). 두 컴포넌트가 예외 경로를 따로 들고 있으면 또 이렇게 하나만
  갱신되는 사고가 재발할 수 있어 `web/src/lib/authGateExemptPaths.ts`로 공유 상수화.
- **자동재생 배속 슬라이더가 바로 안 먹는 버그**: `mobile/App.tsx`의 `AUTOPLAY_SET_RATE`
  핸들러가 "다음 세그먼트부터 적용"으로 의도적으로 설계돼 있었으나(주석 그대로 있었음),
  실사용 체감상 "정지 후 재생해야 바뀐다"로 리포트됨 — 재생 중이면 지금 읽는 단어를 새
  배속으로 즉시 다시 시작하도록 변경(세그먼트 처음부터 살짝 반복되는 트레이드오프는 배속
  슬라이더에서 흔히 허용되는 수준으로 판단).
- **책 상세(개인/공용/관리자) 목차 타이틀이 아이콘 영역을 뚫고 나가는 버그**: 타이틀 `<span>`에
  `flex-1`만 있고 `min-w-0`/`truncate`가 없어 긴 제목(특히 공백 없는 토큰)이 flex 아이템의
  기본 `min-width:auto` 때문에 축소되지 않고 형제 버튼 영역까지 침범했다. 세 화면
  (`BookDetailPage.tsx`, `PublicBookViewPage.tsx`, `AdminBookDetailPage.tsx`) 전부 동일
  패턴이라 같이 수정.
- **"목차별 재생은 있는데 책 전체 재생이 없다"**: 사실 각 목차의 "듣기" 버튼이 이미 그 책의
  전체 목차를 자동재생 세션으로 넘기고 있어(그 목차부터 시작해 다음 목차로 계속 이어짐)
  기능 자체는 있었지만, 진입점이 "아무 목차나 눌러야 시작됨"이라 눈에 띄지 않았다. 개인/공용
  책 상세 헤더에 "전체 듣기" 버튼(0번 목차부터 시작)을 추가해 명시적인 진입점으로 만듦.
- **딥링크가 실제로는 안 열리는 문제 — 실제 코드 버그 발견**: `LoginPage.tsx`의 "링크 로그인"
  (매직 링크, 초대와 무관한 일반 로그인) 탭이 `emailRedirectTo: window.location.origin`
  (경로 없는 루트)으로 보내고 있었는데, AASA/App Links에 등록한 경로는 `/master/accept`·
  `/reset-password` 둘뿐이라 이 루트 경로는 애초에 매칭 대상이 아니었다 — 무조건 브라우저로
  열릴 수밖에 없는 실제 버그였다. `${origin}/login`으로 리다이렉트하도록 수정(`/login`은
  이미 있는 라우트이고, 세션이 잡히면 `LoginPage`의 `if (user) return <Navigate to="/" />`가
  그대로 홈으로 보내줘 별도 처리 불필요) + AASA/`app.json`의 Android `intentFilters`/
  `mobile/App.tsx`의 `DEEPLINK_PATHS`에 `/login` 추가.
  - **다만 이것만으로 완전히 설명되진 않는다**: `/master/accept`·`/reset-password`는 원래도
    등록돼 있었으므로, 이 두 경로까지 여전히 브라우저로 열렸다면 (a) iOS는 지난번 빌드가
    Associated Domains 엔타이틀먼트 누락으로 실패해 애초에 반영된 빌드가 없었고, (b) Android는
    이번 `/login` 추가분이 새 빌드에 반영되기 전이라 아직 완전한 테스트가 아니었을 가능성이
    크다. 그 외에 원천적으로 남는 변수: 일부 메일 앱(Gmail 자체 브라우저 제외, Naver
    메일/Kakao메일 등 자체 인앱 브라우저를 쓰는 앱)은 링크를 자체 웹뷰로 열어버려 OS의 딥링크
    라우팅을 아예 안 태우는 경우가 있다 — 이건 앱 설정으로 해결 불가능한 메일 클라이언트 쪽
    한계라, 재현 시 어떤 메일 앱으로 열었는지 확인 필요.
- **공용 책장 "담기" 여부**: 사용자가 "지금이 나은 것 같다"고 하면서도 확정 요청은 아니어서
  코드는 변경하지 않음(현재 공용 책장은 `PublicBookViewPage.tsx` 주석대로 "읽기 전용, 원본
  참조 방식"으로 의도적으로 설계돼 있었음 — 공용 단어장과 달리 책은 개인 학습 진행 상태 같은
  걸 추적하지 않아 "담기"의 의미가 다르다). 사용자 확인 후 필요하면 별도로 처리.
- **개인정보처리방침/이용약관 영향 검토**: 위 5건 전부 새로운 개인정보 수집 항목, 제3자 처리,
  동의 절차 변경이 없어 **법률 문서 수정 불필요**로 판단(딥링크 경로 추가는 이미 반영된
  "메일 발송" 처리 목적 범위 내).
...
- EAS iOS 빌드 완료. Android는 사용자가 별도 방식으로 빌드 진행 예정 — 빌드 완료 후 동일한
  QA(A~F, G~T)를 Android에서도 재확인 필요(`docs/launch/PHASE1_POLICY.md` §10 5/5-1단계 참고).
- self-signup 계정 재확인 완료(§9 쿼리 재실행 포함).

---

## 2026-09-10

### 비밀번호 변경/재설정 기능에 따른 1차 출시 정책 문서 정합성 점검(신규 정책 아님, 표현 일치화)

- **배경**: 비밀번호 변경/재설정 기능(같은 날 앞선 항목) 추가로 개인정보처리방침/이용약관/동의
  UI/QA 체크리스트가 실제 구현과 어긋나지 않는지 점검. 사용자가 명시적으로 "기존 개인정보 동의/
  이용약관 정책은 변경하지 않는다"고 범위를 한정 — 새 수집항목·새 동의 UI·새 수탁자를 추가하는
  게 아니라 기존 조항의 표현을 실제 기능과 맞추는 작업으로만 진행.
- **개인정보처리방침**: §1 Master 처리 목적에 "계정 관리(비밀번호 변경·재설정 포함)" 추가(새 개인정보
  항목 아님 — 이메일 주소는 §2에 이미 있음). §7/§8의 Resend 위탁업무·이전목적 문구를 "Master 초대
  및 인증 관련 이메일 발송" → "Master 초대, 인증 및 계정(비밀번호 재설정 등) 관련 이메일 발송"으로
  수정(새 수탁자 추가 아님 — 기존 Resend가 이미 이 이메일도 보내고 있었을 뿐). §12(안전성
  확보조치)의 "비밀번호는 Supabase Auth가 해시 처리, 평문 미저장" 원칙은 이미 있었고 변경 없음.
- **이용약관**: 제8조(계정 관리)에 "비밀번호를 포함한 계정 정보를 안전하게 관리할 책임"과 "서비스
  제공 절차로 비밀번호 변경·재설정 가능"을 추가(조항 번호는 유지, 신설 아님). Supabase Email OTP
  Expiration(3600초) 같은 기술 설정값은 의도적으로 약관/방침에 넣지 않음 — 사용자 지시대로 내부
  보안/운영 명세(`docs/launch/PHASE1_POLICY.md`, 이 문서)에서만 관리.
- **동의 UI — 변경 없음 확인**: `MasterAcceptPage.tsx`의 체크박스는 여전히 이용약관 동의 + 만 14세
  확인 2개뿐이고, `user_policy_agreements.agreement_type` CHECK 제약(마이그레이션 43)도
  `'terms' | 'age_eligibility'` 그대로다 — 비밀번호 재설정 관련 체크박스나 `password_reset`
  agreement_type을 추가하지 않았음을 재확인(애초에 구현 때도 추가한 적 없음).
- **QA 체크리스트 보강**: `docs/launch/PHASE1_POLICY.md` §10에 "5-1단계 — 비밀번호 변경/재설정
  보안 QA"를 신설(G~T, 14개 항목) — 가입/미가입 이메일 요청, 정상/만료/재사용/변조 링크, 일반
  세션으로 `/reset-password` 직접 접근 차단, recovery 세션에서만 변경 가능, 신규/기존 비밀번호
  로그인, Redirect URL, Resend 실제 수신·Click Tracking 링크 변형 여부까지 포함.
- **변경하지 않은 것**: 개인정보 수집 항목(§2), 국외이전 표의 리전/근거(§8), 만 14세 정책(§14),
  동의 UI 구조, `user_policy_agreements` 스키마 — 전부 그대로.
...
- 사용자가 직접 확인해 제공한 값을 반영: 운영자 성명 윤화현, 연락처/문의 채널 yunhwahyun@gmail.com,
  Vercel 법인명 Vercel Inc., Vercel 프로젝트 리전 **싱가포르(ap-southeast-1, Southeast Asia)**
  — Supabase와 동일 리전으로 확인됨.
- `docs/legal/PRIVACY_POLICY_PHASE1.md`/`web/public/legal/privacy-policy.md`(§8 국외이전 표, §10
  문의채널, §15 문의처, 체크리스트)에 반영. Vercel 행의 "위탁 해당 여부"와 국외이전 법적 근거는
  리전 확정과 별개로 여전히 법무 확인 필요 상태로 남김(임의 판단하지 않음).
- **시행일은 "미정"으로 확인됨** — 실값을 만들어 채우지 않고, "아직 미정, 스토어 제출 시점에
  확정 예정"으로 문구만 갱신(체크리스트는 미완료 상태 유지). 최종 게시 전 반드시 확정 필요.
- `docs/launch/PHASE1_POLICY.md` §6 판단표, §14 미해결 사항 표도 동일하게 갱신.
