...
**수정**: `expandScheduleOccurrences()`가 이제 "일정이 걸치는 기간"과 "조회 범위"가 **겹치는
날짜마다** occurrence를 하나씩 만든다(`pushOverlappingDays()` 신설). 실제 `starts_at`/`ends_at`
값은 그대로 유지하고, 카드가 어느 날짜 헤더 아래 붙을지(`occurrence_date`)만 날짜별로 바뀐다.
당일 일정(반복이든 아니든 걸치는 기간이 하루뿐인 절대다수 케이스)은 겹치는 날이 하루뿐이라
전과 완전히 동일하게 1건만 나온다 — 회귀 없음, `tsx`로 직접 케이스 7개 검증(당일 반복 회귀,
범위 밖 제외 회귀, 다중일자 종일 노출, 다중일자+주간뷰 확장 등).

**알려진 한계(의도적으로 안 고침)**: 반복 자체가 걸치는 기간보다 짧은 간격으로 도는 극단적인
설정(예: 2일짜리 종일 일정을 "매일" 반복)은 회차끼리 서로 겹쳐서 같은 날짜에 카드가 2장 뜰 수
있다 — 이건 버그가 아니라 그 설정대로라면 실제로 그 날 두 회차가 동시에 "진행 중"인 게 맞아서
그렇다(진짜 일반적이지 않은 구성이라 별도 처리 안 함).

**상세**: `docs/UI_FLOW.md` "일정(`/schedules`)" 참고.
**검증**: `tsx`로 시나리오 7개 직접 실행 확인, `tsc --noEmit`/`npm run build`/`eslint` 통과.
