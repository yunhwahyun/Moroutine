...
- **2차 재시도(같은 날)**: 사용자가 iOS date/time input 버그의 표준 처방(`appearance-none`으로
  ...
  이 둘이 iOS/Android 양쪽 증상의 실제 원인이었을 가능성이 높다. 이번에도 이 환경에는 실기기가
  없어 실제 동작은 검증하지 못했다 — 다만 이 조합은 iOS Safari date/time input 버그의 널리 알려진
  표준 해결책이라 1차 시도보다 훨씬 근거가 탄탄하다.
