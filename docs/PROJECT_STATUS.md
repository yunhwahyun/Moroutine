...
**여러 날짜에 걸친 일정이 조회 범위에서 통째로 안 보이던 버그 수정(2026-09-15):** 종일 일정에
종료 날짜를 계속 노출하도록 바꾼 직후 사용자가 로컬 테스트로 발견 — 9/14~9/15 종일 일정을
만들고 "오늘"(9/15)을 조회하면 일정이 아예 안 보였다. 원인은 `scheduleRepeat.ts`의
`expandScheduleOccurrences()`가 occurrence를 시작일 하루에만 붙이고 그 시작일만 조회 범위와
비교했기 때문 — 시작일이 범위보다 이전이면(예: 어제 시작한 일정) 오늘도 진행 중인데 통째로
빠졌다. 일정이 걸치는 기간과 조회 범위가 겹치는 날짜마다 카드를 하나씩 만들도록 수정, 당일
일정(절대다수)은 회귀 없음을 `tsx`로 케이스 7개 직접 검증. 상세는 `docs/DECISION_LOG.md`
2026-09-15 참고.
