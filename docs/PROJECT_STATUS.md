...
| Phase 17 — `master-invite` 실사용 500 에러 | **원인 미확진, 우회 경로로 대응(2026-07-19)** | 실사용 중 "Edge Function returned a non-2xx status code" 발생, `master_invitations`에 행이 생기지 않아 이메일 발송 단계(`sendInviteEmail`)에서 실패로 추정(SMTP 미설정 가능성, `docs/DECISION_LOG.md` 2026-07-19). 신규 `master-add-existing` Edge Function으로 "이미 가입된 사용자" 한정 이메일 없는 즉시 추가 경로를 배포해 당장의 사용은 가능. **잔여**: 신규 미가입 이메일 초대(`master-invite`)는 여전히 막혀 있을 수 있음 — Dashboard → Edge Functions → Logs에서 실제 에러 확인 후 SMTP 설정 필요 |
