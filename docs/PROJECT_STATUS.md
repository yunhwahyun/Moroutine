...
기다리는 Promise로 변경, 웹 배포만으로 적용). ③ 안드로이드 알림이 몇 분 늦게 오는 문제는
"정확한 알람" 권한 미요청이 원인으로 추정되나 **미해결**(모바일 앱 코드 변경 + 새 EAS 빌드
필요, 사용자 확인 대기). 사용자 공용 책장 상세 목차도 개인/관리자 책장과 동일하게
`ExpandableText`(전체보기/한줄 보기 토글)로 통일. 상세는 `docs/DECISION_LOG.md` 2026-09-16
참고.
