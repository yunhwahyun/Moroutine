...
// 2026-09-10 신설 — 하위 public_words는 ON DELETE CASCADE(마이그레이션 17)로 함께 삭제된다.
export async function deletePublicWordbook(id: string): Promise<void> {
  const { error } = await supabase.from('public_wordbooks').delete().eq('id', id)
  if (error) throw error
}

export async function deletePublicWordbooks(ids: string[]): Promise<void> {
  if (ids.length === 0) return
  const { error } = await supabase.from('public_wordbooks').delete().in('id', ids)
  if (error) throw error
}
...
// 2026-09-10 신설
export async function deletePublicWord(id: string): Promise<void> {
  const { error } = await supabase.from('public_words').delete().eq('id', id)
  if (error) throw error
}

// "비우기" — 단어장은 유지한 채 하위 단어 전체만 삭제.
export async function clearPublicWordbookWords(wordbookId: string): Promise<void> {
  const { error } = await supabase.from('public_words').delete().eq('wordbook_id', wordbookId)
  if (error) throw error
}
