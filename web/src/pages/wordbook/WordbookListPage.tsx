...
export default function WordbookListPage() {
  ...
  const { data: wordbooks = [], isLoading } = useQuery<WordbookWithStats[]>({
    queryKey: ['wordbooks'],
    queryFn: async () => {
      const [wbRes, passedRes] = await Promise.all([
        supabase.from('wordbooks').select('*').order('created_at', { ascending: false }),
        supabase.from('words').select('wordbook_id').in('status', ['reviewing', 'mastered']),
      ])
      if (wbRes.error) throw wbRes.error
      if (passedRes.error) throw passedRes.error

      const passedMap = (passedRes.data ?? []).reduce<Record<string, number>>((acc, w) => {
        acc[w.wordbook_id] = (acc[w.wordbook_id] ?? 0) + 1
        return acc
      }, {})

      return (wbRes.data ?? []).map((wb) => ({
        ...wb,
        mastered_count: passedMap[wb.id] ?? 0,
      }))
    },
  })
  ...
  const { mutateAsync: createWordbook, isPending, error: createError, reset: resetError } = useMutation({
    ...
    onSuccess: (data) => {
      queryClient.setQueryData<WordbookWithStats[]>(['wordbooks'], (old = []) => [
  ...
  const fetchSelectedWords = async (): Promise<Word[]> => {
    ...
    if (wordbookIds.length > 0) {
      const { data } = await supabase.from('words').select('*').in('wordbook_id', wordbookIds)
      ;(data ?? []).forEach((w: Word) => wordsMap.set(w.id, w))
    }
